fnc_service =
{
    if (diag_tickTime < (uiNamespace getVariable ['tag_service',-1])) exitWith {
			hint (format ['Please wait %1 more seconds.',(round ((uiNamespace getVariable ['tag_service',-1]) - diag_tickTime))]);
		};
		_cooldown = 600; 	// cooldown duration in seconds
		uiNamespace setVariable ['tag_service',(diag_tickTime + _cooldown)];
...
_vehicle = "C_Offroad_01_repair_F" createVehicle (getPos player);
