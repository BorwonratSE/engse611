fnc_vehicledrop =
{
    if (diag_tickTime < (uiNamespace getVariable ['tag_tank',-1])) exitWith {
			hint (format ['Please wait %1 more seconds.',(round ((uiNamespace getVariable ['tag_tank',-1]) - diag_tickTime))]);
		};
		_cooldown = 600; 	// cooldown duration in seconds
		uiNamespace setVariable ['tag_tank',(diag_tickTime + _cooldown)];
...
_vehicle = "B_MBT_01_TUSK_F" createVehicle (getPos player);
